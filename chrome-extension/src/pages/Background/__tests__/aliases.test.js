import {
    askClaimAlias,
    checkPasswordAlias,
    createClaimAlias, getEncryptedPasswordAlias, getEncryptedPasswordCSAlias, importWalletAlias,
    sendErrorToCSAlias,
    sendErrorToPopupAlias,
    setPasswordAlias, shareActiveAccountAlias
} from "../aliases";
import {sendErrorToCS, sendErrorToPopup} from "../../../commons/errors";
import { pwService } from "../pwService";
import configureStore from "redux-mock-store";
import thunk from "redux-thunk";
import {GET_PASSWORD_CS_ALIAS, SET_ACTIVE_ACCOUNT, SET_CLAIM, SET_SESSION, SET_WALLET} from "../actionTypes";
...
describe('aliases', () => {
   ...
   it('should send error to CS', () => {
    ...
    it('should check the password', async () => {
        decrypt.mockImplementation(jest.fn(() => ({
            'export': jest.fn(() => ''),
            accounts: [{
                label: 'test',
    ...
    it('should share claim', async () => {
        ...
        await store.dispatch(askClaimAlias({schemaName, issuerDID, verifierName}));
        const actions = store.getActions();
